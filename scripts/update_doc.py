...
SAVE_DIR = "docs"
LONGPORT_DOC = "Longbridge_LLMs.md"
AKSHARE_DOC = "AkShare_LLMs.md"
TRADEFLOW_DOC = "TradeFlow_LLMs.md"

class TradeFlowDocGenerator:
    """
    ...
    def _generate_tree(self, dir_path: Path, prefix: str):
        ...
        items = sorted(
            [
                ...
                if not i.startswith(".") and not i.startswith("__")
            ]
        )
    ...
    def _process_function(self, node: ast.FunctionDef, is_method=False, level=4):
        ...
        if docstring:
            ...
            cleaned_doc = "\n".join(
                [f"{indent}{line}" for line in docstring.splitlines()]
            )
            self.content.append(f"\n{cleaned_doc}")
...
def fetch_and_update_longbridge_wiki():
    ...
    wiki_path = os.path.join(ROOT_DIR, SAVE_DIR, LONGPORT_DOC)
...
def fetch_and_update_akshare_wiki():
    ...
    wiki_path = os.path.join(ROOT_DIR, SAVE_DIR, AKSHARE_DOC)
    content = f"DOC: {url}"
    print(f"Updating {wiki_path}...")
    with open(wiki_path, "w", encoding="utf-8") as f:
        f.write(content)
...
if __name__ == "__main__":
    ...
    generator = TradeFlowDocGenerator(
        Path(ROOT_DIR).joinpath("app"),
        Path(ROOT_DIR).joinpath(SAVE_DIR).joinpath(TRADEFLOW_DOC),
    )
    generator.generate()
